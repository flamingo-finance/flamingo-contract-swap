    ...
    partial class FlamingoSwapAggregatorContract
    {
        ...
        private static void SafeTransfer(UInt160 token, UInt160 from, UInt160 to, BigInteger amount, byte[] data = null)
        {
            try
            {
                ...
                Assert(result, "Transfer Fail in Aggregator", token);
            }
            catch (Exception)
            {
        ...
        private static void RequestTransfer(UInt160 token, UInt160 from, UInt160 to, BigInteger amount, byte[] data = null)
        {
            try
            {
                ...
                Assert(result, "Transfer Not Approved in Aggregator", token);
                Assert(balanceAfter == balanceBefore + amount, "Unexpected Transfer in Aggregator", token);
            }
            catch (Exception)
            {
                Assert(false, "Transfer Error in Aggregator", token);
